...
import {
  contractAddresses, supportedLendingInvestmentPools,
  sashimiAddress, wethAddress, wethSashimiLpPAddress,
  wethName
} from "../../../sushi/lib/constants";
import {getEthChainInfo} from "../../../utils/getEthChainInfo";
import {getInvestmentLendingContract} from "../../../utils/investmentLending";
import {
  useTokenWethPriceByRouter,
  useTokenWethPriceByRouterByConverter,
  IConverterOptions
} from "../../../hooks/useTokenWethPriceByRouter";
...
type IInvestment = {
  ...
  icon: string
  converterOptions?: IConverterOptions
}
  ...
  return (
    <StyledCards>
      <StyleSubTitle>
        <a
          target="_blank"
          href={`${exploreURL}/address/${contractAddresses.investment[1]}`}>
...
const InvestmentCard: React.FC<InvestmentCardProps> = (
  ...
  const tokenWethPrice = useTokenWethPriceByRouterByConverter(
    sashimiRouterContract, investment.lpAddress, investment.depositAddress, investment.converterOptions);
  ...
  const fetchData = useCallback(async () => {
    ...
    console.log('earnedCurrent',
      tokenWethPrice.toString(), investment.lpAddress,
      earnedCurrent[0].toString(), earnedCurrent[1].toString(),
      sashimiBalanceOfProvider.toString());
        ...
        <CardContent>
          <StyledContent>
            ...
            { investment.providerAddress ? <a
              target="_blank"
              href={`${exploreURL}/address/${investment.providerAddress}`}>
              Click To Review The Contract
            ...
            <StyledInsight>
              ...
              <span>
                {getBalanceNumber(supplyProfitEthValued, 18).toFixed(8) || '-'} {wethName.toUpperCase()}
              </span>
            </StyledInsight>
            <StyledInsight>
              <span>Farm Profit</span>
              <span>
                {getBalanceNumber(farmProfitEthValued, 18).toFixed(8) || '-'} {wethName.toUpperCase()}
              </span>
            </StyledInsight>
